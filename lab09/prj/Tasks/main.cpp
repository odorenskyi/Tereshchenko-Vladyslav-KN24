...
int main() {
    ...
    while (true) {
        cout << "Введіть символ (d/g/h/j, k/к для виходу): ";
        getline(cin, input);
        ...
        switch (cmd) {
            ...
            case 'g': {
                ...
                cin >> kilowatts;
                cin.ignore();
                cout << "Сума до сплати: " << calculateElectricityBill(kilowatts) << " грн" << endl;
                break;
            }

            case 'h': {
                double windSpeeds[6];
                cout << "Введіть 6 значень сили вітру (м/с): ";
                for (int i = 0; i < 6; ++i) cin >> windSpeeds[i];
                cin.ignore();
                cout << "Мінімальний бал за шкалою Бофорта: " << getMinBeaufortScale(windSpeeds, 6) << endl;
                break;
            }

            case 'j': {
                int N;
                cout << "Введіть число N (0 до 500700): ";
                cin >> N;
                cin.ignore();
                if (N >= 0 && N <= 500700) {
                    cout << "Результат аналізу: " << analyzeBinary(N) << endl;
                } else {
                    cout << "Помилка: число поза діапазоном." << endl;
                }
                break;
            }
            ...
            default:
                cout << "\aНеправильне введення символу." << endl;
        }
    }
}
